...
resource "azurerm_network_security_rule" "custom_rules" {
    ...
    name = format("%s%s%s",(lookup(var.custom_irules[count.index],"access",var.internal_rules[lookup(var.custom_irules[count.index],"rule","none")][7],"Allow")),(split(",", replace(lookup(var.custom_irules[count.index], "from_port", "*"), "*", "0-65535"))),(lookup(var.custom_irules[count.index],"direction",var.internal_rules[lookup(var.custom_irules[count.index],"rule","none")][6], "Any")))
...
resource "azurerm_network_security_rule" "custom_source_rules" {
    ...
    #source_address_prefix  = length(lookup(var.custom_irules_source[count.index], "source_application_security_group_ids", [])) == 0 ? lookup(var.custom_irules_source[count.index], "source_address_prefix", "*") : "*"
    source_address_prefix  = length(lookup(var.custom_irules_source[count.index], "source_application_security_group_ids", [])) == 0 ? lookup(var.custom_irules_source[count.index], "source_address_prefix", "*") : "*"
    destination_address_prefix = length(lookup(var.custom_irules_source[count.index], "destination_application_security_group_ids", [])) == 0 ? lookup(var.custom_irules_source[count.index], "destination_address_prefix", "*") : "*"
