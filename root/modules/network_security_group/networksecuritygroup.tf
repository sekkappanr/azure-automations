...
resource "azurerm_network_security_rule" "custom_rules" {
    ...
    name = format("%s%s%s",(lookup(var.custom_irules[count.index],"access",var.internal_rules[lookup(var.custom_irules[count.index],"rule","none")][7],)),(split(",", replace(lookup(var.custom_irules[count.index], "from_port", "*"), "*", "0-65535"))),(lookup(var.custom_irules[count.index],"direction",var.internal_rules[lookup(var.custom_irules[count.index],"rule","none")][6], )))
